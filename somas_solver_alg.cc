...
void FootPrint::addElem(BlockTensor *block, const size_t &offset) {
  ...
  if (m_foot_print_next_ == nullptr) {
    ...
    std::cout << "--DEBUG-- " << "Creating footprint at offset: " << m_offset_ << std::endl;
  }

  size_t offset1 = offset;
  SomasSolverTensorDescPtr tensor = block->m_start_tensor_;
  assert(tensor != nullptr);
  std::cout << "--DEBUG-- " << "Allocating block: " << tensor->index_ << " in offset: " << offset << std::endl;
  auto sol_id = block->m_current_sol_;
  if (block->offsets_.find(sol_id) != block->offsets_.end()) {
    std::cout << "--WARNING-- " << "Warning addElem: Offset overwritten at solution " << sol_id << " for block "
              << block->m_start_tensor_->index_ << std::endl;
  }
  ...
  while (tensor) {
    ...
    std::cout << "--DEBUG-- " << tensor->index_ << " " << tensor->size_ << " " << tensor->offset_ << std::endl;
...
void FootPrint::printStats() {
  std::cout << "--DEBUG-- " << "Footprint blocks: " << m_starts_.size() << " \toffset: " << m_offset_ << std::endl;
}
  ...
  for (auto &block : *block_tensors_v) {
    if (!block.m_bre_allocate_) {
      ...
      if (block.offsets_.find(aux_id) != block.offsets_.end()) {
        std::cout << "--WARNING-- " << "Warning: Offset overwritten at solution " << aux_id << " for block "
                  << block.m_start_tensor_->index_ << std::endl;
      }
    ...
    while (!bpushed) {
      ...
      if (p->Next() != nullptr) {
        ...
        std::cout << "--WARNING-- " << "Internal Error: Could not allocate memory for tensor: " << tensor->index_
                  << std::endl;
        return false;
      }
    }
