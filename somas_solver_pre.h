...
class DynamicBitSet {
  ...
  [[nodiscard]] inline size_t GetIndex(size_t index) const { return index / bit_width_; }

  [[nodiscard]] inline uint64_t GetBitMask(size_t index) const {
 ...
 public:
  ...
  void SetBitTrue(size_t index, bool log = false) {
    if (log) {
      std::cout << "--INFO-- " << GetIndex(index) << " " << GetBitMask(index) << std::endl;
    }
  ...
  [[nodiscard]] bool IsBitTrue(size_t index) const { return (bit_[GetIndex(index)] & GetBitMask(index)) != 0x0; }

  [[nodiscard]] size_t CountOnesNum() const {
    size_t ret = 0;
...
class SomasSolverPre {
 public:
  ...
  [[nodiscard]] size_t GetMaxOffset() const { return max_offset_; }

  Status Solving(TensorsDescMap *ptensors, const std::vector<DynamicBitSet> *pConstraints,
  ...
  static Status CheckTensors(const TensorsDescMap *pTensors, uint32_t index1, uint32_t index2);

  static Status AddContiguousInfoInMap(const vector<vector<size_t>> &continuous_v, TensorsDescMap *pTensors);

  static Status AddContiguousInfoInMultiMaps(const vector<vector<size_t>> &continuous_v,
                                             vector<TensorsDescMap> *vecTensorsMap, const TensorsDescMap *pTensors);

 private:
  size_t max_offset_;

  [[nodiscard]] static vector<TensorsDescMap> CreateTensorsMaps(const TensorsDescMap &tensors, size_t total_sol);
};
