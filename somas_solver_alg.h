...
class Interval {
 public:
  ...
  [[nodiscard]] bool intersect(const Interval &i) const { return (in(i.m_a_) || in(i.m_b_)); }
  [[nodiscard]] bool in(const size_t &a) const { return ((a > m_a_) && (a < m_b_)); }
  [[nodiscard]] Interval intersection(const Interval &i) const {
    if (m_a_ < i.m_a_) {
  ...
  [[nodiscard]] bool contains(size_t width) const { return (m_b_ - m_a_) >= width; }
  [[nodiscard]] bool contains(const Interval &a) const { return ((a.m_a_ >= m_a_) && (a.m_b_ <= m_b_)); }
...
class FootPrint : public std::enable_shared_from_this<FootPrint> {
 public:
  ...
  void Destroy();
  size_t getOffset() const { return m_offset_; }
