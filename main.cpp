...
Status Solve(std::vector<SomasTensorPtr> &tensors_list, std::vector<DynamicBitSet> reuse_matrix) {
  ...
  auto status = somas_solver_->Solving(&solver_tensor_desc_map_, &reuse_matrix, EmptyContiguousConstraintsVec, true);
...
void SaveToCSV(const std::vector<SomasTensorPtr> &tensors_list) {
  ...
  if (outfile_out.is_open()) {
    ...
    for (const auto &tensor : tensors_list) {
      outfile_out << tensor->GetId() << "," << tensor->lifetime_.start_ << "," << tensor->lifetime_.end_ << ","
